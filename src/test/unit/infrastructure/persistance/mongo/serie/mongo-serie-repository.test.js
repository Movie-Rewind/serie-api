...
describe('Mongo Serie Repository', () => {
    ...
    beforeEach(() => {
        mongoSerieRepositoryMock = new MongoSerieRepository({
            mongoDbHandler: mongoDbHandlerMock,
            serieDocumentParser: serieDocumentParserMock
        });
    });
    it('Save function ', async () => {
        const serieDomain = new Serie(serieMock);
        ...
        serieDocumentParserMock.toDocument.mockReturnValue(serieMock);

        await mongoSerieRepositoryMock.save(serieDomain);
        ...
        expect(dbInstanceMock.insertOne).toHaveBeenCalledWith(serieMock);
    ...
    it('FindByOmdbId function success', async () => {
        ...
        mongoDbHandlerMock.getInstance.mockReturnValue(dbInstanceMock);
        dbInstanceMock.collection.mockReturnValue(dbInstanceMock);
        dbInstanceMock.findOne.mockReturnValue(serieMock);
        serieDocumentParserMock.toDomain.mockReturnValue(serieDomain);

        const response = await mongoSerieRepositoryMock.findByOmdbId(serieMock.omdb_id);

        expect(mongoDbHandlerMock.getInstance).toHaveBeenCalledTimes(1);
        expect(dbInstanceMock.collection).toHaveBeenCalledTimes(1);
        expect(dbInstanceMock.collection).toHaveBeenCalledWith(collection);
        expect(dbInstanceMock.findOne).toHaveBeenCalledTimes(1);
        expect(dbInstanceMock.findOne).toHaveBeenCalledWith({omdb_id: serieMock.omdb_id});
        expect(response).toBe(serieDomain);
    });
